 ...
 * 在 自己的代码 中 ，左右指针分别是 left 和 right
 *********************************************************************/
namespace DSAA
{
...
ostringstream& trv(ostringstream& ss, const NODE_T* t)
{
    ...
    return out(trv(trv(ss,t->left),t->right), *t);
}
...
public:
    ...
    void outsubt(ostream& os, NODE_T* t, string s, char e) const
    {
        if (t != nullptr)
        {
             ...
             * 缩进增加, 遍历左子树
             ****************************************/
            outsubt(os, t->left, s+char(e^x)+Indent, '|');
        }
    }
